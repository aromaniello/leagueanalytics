...
namespace :league_data do
  desc "update items from data dragon api in yaml file"
  task :update_items => :environment do
    ...
    item_data = YAML.load(File.read("app/data/item_stats.yml"))
    ...
    api_data.each do |code, item|
      ...
      item_data[item_name]["code"] = code.to_i
      item_data[item_name]["cost"] = api_data[code]["gold"]["total"]
